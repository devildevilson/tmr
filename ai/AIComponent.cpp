...
AIBasicComponent::AIBasicComponent(const CreateInfo &info) : EntityAI(EntityAI::CreateInfo{info.components}), internalIndexVal(SIZE_MAX) {
  aiData.r = info.radius;
  aiData.vertex = info.currentVertex;
  aiData.lastVertex = info.currentVertex;
  
  if (aiData.vertex != nullptr) aiData.vertex->addObject(this); // для базового компонента по идее это обязательно
}
...
AIComponent::AIComponent(const CreateInfo &info)
  : AIBasicComponent({info.radius, info.currentVertex, info.components}),
    tree(info.tree),
    runningNode(nullptr),
...
void AIComponent::updateAIData() {
  ...
  if (usrData->trans != nullptr) {
    usrData->trans->pos().storeu(pos);
    usrData->trans->rot().storeu(dir);
  }
  
  if (usrData->phys != nullptr) {
    usrData->phys->getVelocity().storeu(vel);
    objectIndex = usrData->phys->getIndexContainer().objectDataIndex;
    
    auto cont = usrData->phys->getGround();
    if (cont != nullptr) {
      ...
      vertex_t* vertex = data->vertex;
      if (aiData.vertex != vertex) {
        vertex->addObject(this);
        if (aiData.vertex != nullptr) aiData.vertex->removeObject(this);
        else if (aiData.lastVertex != nullptr) aiData.lastVertex->removeObject(this);
