...
// по идее почти все работы в движке у нас строго определены (по количеству)
// и в идеале мы можем построить макет вычислений 
// макет вычислений представляет собой последовательные списки задач
// последовательность означает то что нельзя вычислять параллельно
// внутри списка задачи могут паралельно исполняться
// причем почти все списки мы можем задать с самого начала 
// осталось придумать способ подождать все потоки, тут наверное ничего сложного
