...
namespace devils_engine {
  namespace render {
    image_container::image_pool::image_pool(yavf::Device* device, const utils::extent_2d &img_size, const uint32_t &mips) {
      image = device->create(
    ...
    image image_container::get_image(const size_t &pool_index) {
      if (pool_index >= slots.size()) return {UINT32_MAX};
      const uint32_t free_index = slots[pool_index].occupy_free_index();
      if (free_index == UINT32_MAX) return {UINT32_MAX};
      return create_image(pool_index, free_index, 0, false, false); //  {static_cast<uint32_t>(pool_index), free_index}
    }
    ...
    void image_container::create_pool(const utils::extent_2d &img_size, const uint32_t &mips) {
